...
def main() -> None:
    parser = argparse.ArgumentParser(description='Run exactly one evaluation language, checkpoint it, and exit.')
    ...
    progress = LanguageCheckpointWriter(run, 'evaluation', len(language_order), resume=True)
    log.info('Launching isolated evaluation language subprocess: %s', args.language_code)
    try:
        ...
        log.info(
            'Evaluation language subprocess exiting; OS will release all RAM/MPS allocations: %s',
            args.language_code,
        )
